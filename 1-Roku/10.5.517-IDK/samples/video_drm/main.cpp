...
{
  // IDK's libcurl is 7.66.0, so it isn't necessary to request SSL.  You'll note
  // documentation says this API is not thread-safe, and should be called only
  // when there are no other threads running:
  //
  ...
  RokuDrm_Eng* drm;                                        // DRM additions
  RokuDrm_Ses* drm_session;                                // to setup Widevine
  RokuDrm_EngCreate("widevine", nullptr, 0, &drm);         // engine, session
  RokuDrm_EngCreateSession(drm, nullptr, 0, &drm_session); // and license
  auto ok = AcquireWidevineDRMLicense(drm_session);        // for this sample.
  if (!ok) {
    RokuDrm_SesDestroy(drm_session);
    RokuDrm_EngDestroy(drm);
    curl_global_cleanup();
    return;
  }
  ...
  video->startBuffering(decoderVideo, drm, drm_session); // DRM
  audio->startBuffering(decoderAudio, drm, drm_session); // addition
  ...
  RokuDrm_SesDestroy(drm_session); // DRM
  RokuDrm_EngDestroy(drm);         // addition
  curl_global_cleanup();
}

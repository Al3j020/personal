...
/* agraham@roku.com: The timedlock feature of mutex needs to remain disabled
   until timed_mutex/shared_timed_mutex/recursive_timed_mutex are ported to use
   a mechanism that isn't susceptible to clock changes. */
...
/* agraham@roku.com: The condition variable needs to use the monotonic clock,
   so it needs to use the init func. */
...
__gthrw(pthread_condattr_init)
__gthrw(pthread_condattr_setclock)
__gthrw(pthread_condattr_destroy)
  ...
  if (__gthread_active_p ())
    {
      ...
      __r = __gthrw_(pthread_condattr_init) (&__attr);
      if (!__r)
	__r = __gthrw_(pthread_condattr_setclock) (&__attr, CLOCK_MONOTONIC);
      if (!__r)
	__r = __gthrw_(pthread_cond_init) (__cond, &__attr);
      if (!__r)
	__r = __gthrw_(pthread_condattr_destroy) (&__attr);
      return __r;
    }
  return 0;
}
